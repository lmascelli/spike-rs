...
impl DataSpace {
    ...
    pub fn select_slab(&self, start: &[u64], offset: &[u64]) -> Result<DataSpace, String> {
        if start.len() != self.dims.len() || start.len() != offset.len() {
            Err(format!(r#"DataSpace::select_slab: invalid selection from {:?} with offset {:?}
            have different rank than dataspace with dimension {}"#, start, offset, self.dims.len()))
        } else {
            let mut valid = true;
            for dim in 0..start.len() {
                if start[dim] as usize >= self.dims[0] || (start[dim] + offset[dim]) as usize >= self.dims[0] {
                    valid = false;
                    break;
                }
            }
            if valid {
                ...
                Ok(Self::parse(unsafe { H5Screate_simple(start.len() as i32, offset.as_ptr(), null())})?)
            }
             else {
                Err(format!(r#"DataSpace::select_slab: slab starting from {:?} with offset {:?}
                is out of bounds of dataspace with dimension {:?}"#, start, offset, self.dims))
             }
    ...
    pub fn select_row(&self, row: usize) -> Result<DataSpace, String> {
        if self.dims.len() != 2 {
            Err(r#"DataSpace::select_row: select_row is valid only for bidimensional dataspaces"#.to_string())
        } else {
